  ...
  return (
    ...
    { mainPost.length > 0 && <div className="flex flex-col items-center md:flex-row">
        <div className="flex flex-col items-start justify-center w-full h-full mb-6 md:mb-0 md:w-1/2">
          ...
          md:space-y-5"
          >
            <h2 className="text-2xl font-bold leading-none lg:text-5xl">
            {mainPost[0].title}
            </h2>
            
            {mainPost[0].description && <p className="text-sm font-medium inline">{mainPost[0].description}</p>}
            <Link to={mainPost[0]._id}
            className='w-[200px] shadow-sm mt-10 flex  items-center justify-center rounded-md border border-transparent bg-indigo-600 px-8 py-3 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2'>Подробнее</Link>
...
{otherPosts.length > 0  ?        <ul className="grid md:grid-cols-8 lg:grid-cols-12 sm:px-1 gap-x-8 gap-y-16">
         {otherPosts.map(el => <BlogCard post={el}/>)}
        </ul> : <div className='py-40 text-center'>Еще нет записей в блоге.</div>}
      </div>
    </div> : <div className='py-40 text-center'>Еще нет записей в блоге.</div>}
  </div> : <SkeletonBlog/>}</>
  );
0};
