  ...
  return (
    <div className="py-4">
      ...
      <p className="antialiased font-sans text-sm leading-normal flex items-center gap-1 font-normal text-blue-gray-600 mb-4">
            Количество заказов: <strong>{userOreders(orders).length}</strong>
          </p>
      <div className=' flex flex-col gap-4'>

        {orders && orders.length > 0 ? (
          userOreders(orders).map((el, index) => (
            <div key={el._id} className='mdOnly:w-[450px] relative border-[1px] border-gray-300 rounded-md flex flex-col justify-center p-4 items-center gap-4'>
            ...
            {el.order.products && 
            <div className='flex flex-wrap justify-between'>{el.order.products.map((el, index) => 
            <div key={index} className=' flex p-4 items-center gap-4 md:gap-2 w-[250px] md:w-[300px] lg:w-[400px]'>
              <div className='w-[50px] h-[50px] overflow-hidden rounded-xl'>
                <img className='w-[100%] h-[100%] object-cover' src={el.product.mainPhoto} alt={el.product.name} />
              </div>
              <div className="gap-[2px] md:pl-3 md:w-8/12 2xl:w-3/4 flex flex-col justify-around">
              <p className="text-xs leading-3 text-gray-800 md:pt-0 pt-4">
              Код товара:{' '}
              <span className=" font-bold">{el.product.article}</span>
            </p>
            <div
              className="flex items-center justify-between w-full pt-1"
            >
              <Link
                to={`/categories/${categoryURL(el.product.category)}/${
                  el.product._id
                }`}
