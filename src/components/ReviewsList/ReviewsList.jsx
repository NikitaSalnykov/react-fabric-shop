...
export const ReviewsList = ({ productId }) => {
  ...
  const [isModalReviewCreateCommentOpen, setModalReviewCreateCommentOpen] =
    useState(false);
    ...
    <>
      <div className="">
        <div className="my-12 flex items-center justify-between">
          ...
          <button
            onClick={onToggleReviewCreateModal}
            className="items-center shadow-sm max-w-[250px] rounded-md border border-transparent bg-lime-500 px-4 py-3 text-base font-medium text-white hover:bg-lime-600 focus:outline-none focus:ring-2 focus:ring-lime-600 focus:ring-offset-2"
          >
            Оставить отзыв
          </button>
        </div>
          ...
          <div>
            {reviews && productReviews.length > 0 ? (
                  ...
                  (el, index) =>
                    el && (
                      ...
                      >
                        <ReviewCart
                          review={el}
                          openCommentModal={onToggleReviewCreateCommentModal}
                          openImageModal={onToggleFullSizeImageModal}
                        />
                      </li>
                    )
                )}
              ...
              <div className=" text-center p-10 text-gray-500">
                Пока нет отзывов к этом товару
              </div>
            )}
          </div>
        ) : (
